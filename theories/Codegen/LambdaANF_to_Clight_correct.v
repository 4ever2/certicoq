...
Section RELATION.
  ...
  Notation funTy := (Tfunction (cons threadInf nil) Tvoid
                            {|
                              cc_vararg := false;
                              cc_unproto := false;
                              cc_structret := false |}).
...
Notation gcTy := (Tfunction (cons (Tpointer (Tint I32 Unsigned noattr) noattr) (cons threadInf nil)) Tvoid
                            {|
                              cc_vararg := false;
                              cc_unproto := false;
                              cc_structret := false |}).

Notation isptrTy := (Tfunction (cons (Tint I32 Unsigned noattr) nil) (Tint IBool Unsigned noattr)
                               {|
                                 cc_vararg := false;
                                 cc_unproto := false;
                                 cc_structret := false |}).
...
Definition program_isPtr_inv (p:program) :=
  exists b_isPtr name sg, Genv.find_symbol (globalenv p) isptrIdent = Some b_isPtr /\
                          Genv.find_funct (globalenv p) (Vptr  b_isPtr Ptrofs.zero) = Some (External (EF_external name sg) (cons val nil)  (Tint IBool Unsigned noattr)   {| cc_vararg := false; cc_unproto := false; cc_structret := false |}) /\
...
Definition program_gc_inv (p:program) :=
  exists b_gcPtr name sg, Genv.find_symbol (globalenv p) gcIdent = Some b_gcPtr /\
                          Genv.find_funct (globalenv p) (Vptr  b_gcPtr Int.zero) = Some (External (EF_external name sg) (cons (Tpointer (Tint I32 Unsigned noattr) noattr) (cons (Tpointer (Tstruct threadInfIdent noattr) noattr) nil)) Tvoid
                            {|
                              cc_vararg := false;
                              cc_unproto := false;
                              cc_structret := false |}) /\
...
(* OS: Changed returned vs7 into vs7' s.t. the pointers can have changed (but represent the same values in LambdaANF) *)
 Definition program_gc_inv (p:program) :=
  exists b_gcPtr name sg, Genv.find_symbol (globalenv p) gcIdent = Some b_gcPtr /\
                          Genv.find_funct (globalenv p) (Vptr  b_gcPtr Ptrofs.zero) = Some (External (EF_external name sg) (cons (Tpointer val noattr) (cons (Tpointer (Tstruct threadInfIdent noattr) noattr) nil)) Tvoid
                            {|
                              cc_vararg := false;
                              cc_unproto := false;
                              cc_structret := false |}) /\
    ...
    (* k := (Kseq
          ...
          (Kseq
             (Scall None
                (Ecast
                   (Evar f
                      (Tfunction (cons (Tpointer (Tstruct threadInfIdent noattr) noattr) nil) Tvoid
                        ...
                        (Ssequence
                           (Ssequence bef
                                      (Scall None
                                             (Evar gcIdent
                                                   (Tfunction
                                                      (cons (Tpointer uval noattr)
                                                             (cons (Tpointer (Tstruct threadInfIdent noattr) noattr) nil)) Tvoid
                                                      {| cc_vararg := false; cc_unproto := false; cc_structret := false |}))
                                             [Evar finfo0 (Tarray LambdaANF_to_Clight.uval (Z.of_N (N.of_nat (length locs) + 2)) noattr);
                    ...
                    (Sifthenelse
                       ...
                       (Ssequence
                          (Ssequence bef
                                     (Scall None
                                            (Evar gcIdent
                                                  (Tfunction (cons (Tpointer uval noattr) (cons (Tpointer (Tstruct threadInfIdent noattr) noattr) nil)) Tvoid
                                                             {| cc_vararg := false; cc_unproto := false; cc_structret := false |}))
                                            [Evar finfo0 (Tarray LambdaANF_to_Clight.uval (Z.of_N (N.of_nat (length locs) + 2)) noattr);
                                             Etempvar tinfIdent (Tpointer (Tstruct threadInfIdent noattr) noattr)])) aft) Sskip)
                | None => None
